...
class data {
    ...
    getRulesets(page, size) {
        ...
        return new Promise((resolve, reject) => {
            ...
            let rulesetsQuery = this.db.query(updateTableNames("SELECT * FROM {{currentRuleset}} " +
                "ORDER BY name ASC LIMIT $1 OFFSET $2", this.tables), [size, offset] );

            let countQuery = this.db.query(updateTableNames("SELECT count(*) FROM {{currentRuleset}}", this.tables));
