...
QUnit.test( "rename single file", function( assert ) {
	const logger = new ErrorLogger();
	const config = {
		__state : {
			"_debugLogger" : logger,
	...
	rule._run( { file: './src/validator/tests/testDataCSVFile.csv' }).then((result) => {
		...
		assert.equal(zipEntries.length, 1, 'Expect only one file in zip');
		assert.equal(zipEntries[0].name, 'newName.csv', 'Expect only one file in zip');

		done();
	});
});

QUnit.test( "rename multiple files", function( assert ) {
	const logger = new ErrorLogger();
	const config = {
		__state : {
			"_debugLogger" : logger,
	...
	rule._run( { file: './src/validator/tests/world_borders' }).then((result) => {
		...
		zipEntries.forEach((entry) => {
			assert.equal(entry.name.slice(0,-4), 'newName');
		});
